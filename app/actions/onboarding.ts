...
export async function completeOnboarding(data: {
       ...
       // 2. Crear categorías demo (Solamente si no existen)
       const existingCategories = await prisma.category.findMany({ where: { storeId } });
       const categoryNames = ["Bebidas", "Snacks", "Cigarrillos", "Almacén"];

       const categories = [];
       for (const name of categoryNames) {
              let cat = existingCategories.find(c => c.name === name);
              if (!cat) {
                     cat = await prisma.category.create({ data: { name, storeId } });
              }
              categories.push(cat);
       }

       // 3. Crear productos demo realistas (Si no hay productos)
       ...
       // 4. Crear clientes demo (Solo si no hay)
       const customersCount = await prisma.customer.count({ where: { storeId } });
       if (customersCount === 0) {
              await Promise.all([
       ...
       // 5. Crear una sesión de caja abierta (Si no hay una abierta)
